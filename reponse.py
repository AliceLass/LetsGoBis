...
def reponse_velib(meta):
    ...
    origin=meta.origin
    arrival=meta.arrival
    station_origin=meta.velibitinerary.station_origin
    station_arrival=meta.velibitinerary.station_arrival
...
def reponse_autolib(meta):
    ...
    origin=meta.origin
    arrival=meta.arrival
    station_origin=meta.autolibitinerary.station_origin
    station_arrival=meta.autolibitinerary.station_arrival
...
def reponse_transit(meta):
    ...
    for i in range(0,len(steps)):
        reponse += str("Prendre la ligne " + steps[i]["line"] + " de " + steps[i]["type"] + " à la station " + steps[0]["departure_stop"] + " en direction de " + steps[i]["direction"] + ". Descendre à " + steps[i]["arrival_stop"] + "." + "\n")
